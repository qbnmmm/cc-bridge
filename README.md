...
管理台“性能”页（`/performance`）提供性能概览、当前实例的进行中请求和慢请求明细。默认开启，设置 `PERFORMANCE_MONITORING_ENABLED=false` 可停止新增采集，已保存历史仍可查询。

- 仅统计推理 `/v1/messages`，包括鉴权失败、本地拒绝、上游错误、超时、断流与取消；count_tokens、管理请求和遥测不计入。
- 首包是首个上游 body 数据块；首内容是实际思考、正文或工具输入；首正文是非空文本。心跳和 message_start 不计作内容进展。
- 完整耗时从请求进入网关到响应 body 结束；模型结束时间单独记录。HTTP 200 后仍可能发生流错误。
- 概览的 P50/P95/P99、最大值和样本数只使用完整成功记录；无值显示 `—`。平均输出速度包含思考和内容间停顿，非流式或一次批量返回不推算速度。
- 进行中列表每 5 秒刷新，按已持续时间排序并展示无内容进展时长；概览每 60 秒刷新。历史明细支持模型、账号、API Token、实例、结果和耗时筛选。
- 性能记录独立保留当前新加坡自然日及之前 29 日，时间范围默认最近 1 小时；既有计费用量历史的保留策略不变。数据写入 schema v4 的 `request_performance_events`，异步有界批写。
- 进行中请求仅属于当前进程；共享数据库可查询各实例历史。队列丢弃、解析失败和实例重启导致的观测缺口会明确提示；进程崩溃后的未落盘请求不能恢复。
- 监控不保存提示词、响应正文或凭证；管理 API `/admin/performance*` 沿用管理员认证。禁用采集或回滚应用不删除历史表。

这里记录的是网关消费响应时的观测，受网络、压缩缓冲和客户端背压影响；无法直接区分上游内部排队与预填充，也不包含客户端工具执行和完整 Agent 任务。取消/响应释放不能精确归因于客户端。现有 300 秒读取空闲超时与自动遥测字段口径保持原行为。
